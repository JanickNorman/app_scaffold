...
BaseApp.prototype = {
  ...
  showModal: function(templateName, data) {
    this.createModal(() => this.renderTemplate(templateName, data));
  },

  createModal: function(callback) {
    return this.zafClient.context().then(() => {
      var options = {
        location: "modal",
        url: "assets/modal_template.html"
      };
      return this.zafClient.invoke("instances.create", options);
    })
    .then((data) => {
      var instanceGuid = data['instances.create'][0].instanceGuid;
      var modalClient = this.zafClient.instance(instanceGuid);

      var html = callback();
      this.sleep(1000).then(() => {
        modalClient.trigger("show_modal", html);
      })
    });
  },
